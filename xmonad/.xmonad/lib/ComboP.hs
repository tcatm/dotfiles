...
instance (LayoutClass l (), LayoutClass l1 Window, LayoutClass l2 Window) =>
    ...
    doLayout (C2P f w1 w2 super l1 l2 prop) rinput s =
        ...
        in do
            ...
            (wrs1, ml1') <- runLayout (Workspace "" l1 s1) $ if isNothing s2 then rinput else r1
            (wrs2, ml2') <- runLayout (Workspace "" l2 s2) $ if isNothing s1 then rinput else r2
    ...
    description (C2P _ _ _ super l1 l2 prop) = description l1
