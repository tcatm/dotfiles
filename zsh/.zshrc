...
function check_backups {                                                        
  ...
  for each in `ls *timestamp|sed 's/\..*//'`; do                                
    backup_age $each                                                            
  done                                                                          
  popd > /dev/null                                                              
}                                                                               
...
function backup_age {                                                           
  age=$[`date +%s`-`cat $1.timestamp`]                                          
  if test $age -gt $[9*86400]; then                                             
    echo -e "\033[1;31mBackup \033[1;33m$1\033[1;31m is \033[1;36m$[age/86400] days\033[1;31m old!"
  fi                                                                            
}                                                                               
...
SHELL_HOME=~/Desktop/
test "`pwd`" = $HOME && cd $SHELL_HOME

alias cd="HOME=$SHELL_HOME cd" > /dev/null

# named directories
# use like this: cd ~krtek
export krtek=/ssh/krtek/
export rawkore=/ssh/kore/stuff/Fotografie/RAW\ Archiv
...
function openrd {
  export PULSE_SERVER=[fe80::250:43ff:fe2c:1435%eth0]
}

stty ixon

alias syncraw='rsync -avP /home/nils/Fotografie/import/ /ssh/kore/stuff/Fotografie/RAW\ Archiv/'
alias batctl='sudo batctl'
export URBIT_HOME=/home/nils/urbit/urb
